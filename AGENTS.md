...
- **Naming**: Match the owning module and Effect ecosystem: PascalCase modules, schemas, and classes; camelCase functions and values. Preserve domain-standard spellings where they are already canonical rather than imposing one constant style on unrelated work.
...
- Public cross-module abstractions live in flat, concern-named modules with one canonical owner. `internal/*` is private; do not introduce catch-all public buckets.
