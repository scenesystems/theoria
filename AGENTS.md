...
1. **Code changes** deploy by merging to `main`: staging deploys automatically and records a release candidate. Publish new package versions with `Publish Packages`, then separately dispatch `Theoria Production` on `main` with the same `run_id`. Production requires matching npm provenance and package release inputs, and promotes the recorded artifact/configuration without rebuilding. Website-only candidates can reuse matching published packages. Optional `reviewed_run_id` carries a prior staging review forward only through verified version-only changes. Candidate/site artifacts expire after seven days. Never dispatch publication or production without explicit release approval.
