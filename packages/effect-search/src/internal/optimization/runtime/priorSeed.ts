/**
 * Prior trial validation and runtime seed construction for warm-starting optimizations.
 *
 * @since 0.1.0
 */
...
/**
 * Internal seed state for optimization initialization, carrying prior trials and the starting trial number.
 *
 * @since 0.1.0
 * @category models
 */
...
const priorTrialFailure = (index: number, reason: string): InvalidOptimizationConfig =>
  new InvalidOptimizationConfig({
    reason: `Optimization.run priorTrials[${index}] ${reason}`
  })
...
const validatePriorCost = (index: number, cost: number): Effect.Effect<void, InvalidOptimizationConfig> =>
...
): Effect.Effect<RuntimeSeed<SearchSpace.Type<Space>>, InvalidOptimizationConfig> =>
