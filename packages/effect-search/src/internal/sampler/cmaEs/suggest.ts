  ...
  Effect.forEach(state.mean, (value, index) =>
    sampleStandardNormal(rng).pipe(
      Effect.map((roll) =>
        clamp01(Num.sum(
          value,
          Num.multiply(state.sigma, Num.multiply(sqrt(covarianceValueAt(state.covarianceDiag, index)), roll))
        ))
      )
    ))
...
export const suggest = (
  ...
  Effect.gen(function*() {
    ...
    const generations = Arr.makeBy(
      ...
      (generationIndex) =>
        Arr.take(
          Arr.drop(orderedByTrial, Num.multiply(generationIndex, populationSize)),
          populationSize
        )
    )
