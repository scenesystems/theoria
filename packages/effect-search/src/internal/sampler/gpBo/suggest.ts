...
import { rngByTrial } from "../shared/rngByTrial.js"
...
const DEFAULT_ACQUISITION: Name = "ei"
...
const expectedImprovementScore = (mean: number, best: number, variance: number): number => {
  ...
  const improvement = Num.subtract(Num.subtract(best, mean), EXPLORATION_EPSILON)
...
const probabilityImprovementScore = (mean: number, best: number, variance: number): number => {
  ...
  return standardNormalCdf(
    Num.unsafeDivide(Num.subtract(Num.subtract(best, mean), EXPLORATION_EPSILON), standardDeviation)
  )
}
