...
import { initializeRuntime, readStudyState, setRuntimeLifecycle, type StudyRuntime } from "./runtime/runtimeState.js"
...
const persistRuntimeCheckpoint = <Space extends SearchSpace.SearchSpace>(
  ...
  Effect.gen(function*() {
    ...
    const snapshot = StudySnapshot.make(History.values(finalState), metadata)
    yield* StudyStorage.writeIfAvailable(snapshot)
      ...
      Effect.gen(function*() {
        ...
        return yield* withRuntimeCheckpoint(
          ...
          Effect.gen(function*() {
            ...
            const completionSnapshot = StudySnapshot.make(trials, snapshotMetadata)

            yield* setRuntimeLifecycle(runtime, "Completed")
            yield* appendEvent(runtime, StudyEvent.completed({ completionReason }))
            yield* StudyStorage.writeIfAvailable(completionSnapshot)
