...
import { type ArtifactStorageError, InvalidStudyConfig } from "../../SearchError.js"
  ...
  Effect.gen(function*() {
    ...
    yield* SynchronizedRef.updateEffect(state.completionPublishedRef, (published) =>
      Effect.if(published, {
        ...
        onFalse: () => appendEvent(state.runtime, StudyEvent.completed({ completionReason })).pipe(Effect.as(true))
      }))
