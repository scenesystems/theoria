...
export const ask = <Space extends SearchSpace.SearchSpace>(
  handle: Study.Study<Space>
): Effect.Effect<Study.AskedTrial<SearchSpace.Type<Space>>, SearchError> =>
  Effect.gen(function*() {
    ...
    return yield* Option.match(reserved, {
      ...
      onSome: (running) =>
        appendEvent(
          ...
          Effect.as(new Study.AskedTrial({ trialNumber: running.trialNumber, config: running.config }))
        )
    })
  })
