...
export class EventPublisher extends Data.Class<{
  ...
  readonly publish: (event: StudyEvent.Event) => Effect.Effect<void, ArtifactStorageError>
}> {}
 ...
 * @category constructors
 */
export const eventPublisherFromPubSub = (pubsub: PubSub.PubSub<StudyEvent.Event>): EventPublisher =>
  new EventPublisher({
...
const SCHEMA_VERSION: StudyArtifact.Version = "artifact-envelope/v1"
const EVENT_COMPONENT: StudyArtifact.ComponentPath = Arr.make("Study", "events")
...
const eventFromFinalizedTrial = <Config>(trial: Trial.Trial<Config>): Option.Option<StudyEvent.Event> =>
  Trial.matchState({
    Running: () => Option.none(),
