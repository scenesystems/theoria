...
export const ndtr = (value: number): number => {
  ...
  return Match.value(scaled).pipe(
    Match.when(
      (current) => lessThan(current, Num.negate(INV_SQRT_TWO)),
      (current) => Num.multiply(0.5, erfc(Num.negate(current)))
    ),
    Match.when(
      (current) => lessThan(current, INV_SQRT_TWO),
      (current) => Num.sum(0.5, Num.multiply(0.5, erf(current)))
    ),
...
const asymptoticSeries = (state: AsymptoticSeriesState): number => {
  return Match.value(
    Boolean.or(
      lessThanOrEqualTo(Float64.abs(Num.subtract(state.lastTotal, state.rightHandSide)), Number.EPSILON),
      greaterThanOrEqualTo(state.index, 1_024)
    )
  ).pipe(
...
export const logNdtr = (value: number): number =>
  Match.value(value).pipe(
    ...
    Match.when(
      (current) => greaterThan(current, LOG_NDTR_RIGHT_TAIL_THRESHOLD),
      (current) => Num.negate(ndtr(Num.negate(current)))
    ),
    Match.when(
      (current) => greaterThan(current, LOG_NDTR_ASYMPTOTIC_THRESHOLD),
      (current) => Float64.log(ndtr(current))
    ),
...
export const logDiff = (logP: number, logQ: number): number =>
  Match.value(logP).pipe(
    ...
    Match.when((current) => lessThanOrEqualTo(current, logQ), () => Number.NEGATIVE_INFINITY),
...
const newtonRefine = (targetLogNdtr: number, current: number, iteration: number): number => {
  return Match.value(greaterThanOrEqualTo(iteration, NEWTON_MAX_ITERATIONS)).pipe(
    ...
    Match.orElse(() => {
      ...
      return Match.value(lessThan(Float64.abs(delta), tolerance)).pipe(
...
const solveNdtriExp = (value: number): number => {
  const flipped = greaterThan(value, NDTRI_EXP_FLIP_THRESHOLD)
  const normalized = Boolean.match(flipped, {
    onFalse: () => value,
    onTrue: () => Float64.log(Num.negate(Float64.expm1(value)))
  })

  const initialGuess = Match.value(lessThan(normalized, NDTRI_EXP_SWITCH)).pipe(
    Match.when(true, () => Num.negate(Float64.sqrt(Num.multiply(-2, Num.sum(normalized, LOG_SQRT_TWO_PI))))),
    Match.orElse(() =>
      Num.multiply(
        Num.negate(NDTRI_EXP_APPROX_C),
        Float64.log(Float64.expm1(Num.negate(normalized)))
      )
    )
  )
...
export const ndtriExp = (value: number): number =>
  Match.value(value).pipe(
    ...
    Match.when((current) => Boolean.not(isFinite(current)), () => Number.NaN),
