 ...
 * @since 0.1.0
 */
import { Array as Arr, Boolean as Bool, Data, Effect, Equal, Match, Number as Num, Option, Record, Tuple } from "effect"
...
const modelVectorFromConfig = (
  adaptersInput: Iterable<ContinuousAdapter>,
...
export const vectorsFromSplit = (
  adaptersInput: Iterable<ContinuousAdapter>,
  trialsInput: Iterable<{ readonly config: SamplerConfig }>
) => {
  const adapters = Arr.fromIterable(adaptersInput)
...
export const configFromCandidate = (
  adaptersInput: Iterable<ContinuousAdapter>,
  candidateValuesInput: Iterable<number>
): unknown => {
  const adapters = Arr.fromIterable(adaptersInput)
  const candidateValues = Arr.fromIterable(candidateValuesInput)
  return Record.fromEntries(
    Arr.map(Arr.zip(adapters, candidateValues), ([adapter, value]) => Tuple.make(adapter.name, value))
  )
}
...
export const normalizeModelCandidate = (
  adaptersInput: Iterable<ContinuousAdapter>,
  modelCandidateInput: Iterable<number>,
