...
import type { Constraint, TpeOptions } from "../../Sampler.js"
import { InvalidSamplerConfig } from "../../SearchError.js"
import { numberOptionOr } from "../sampler/optionReaders.js"

const defaultNoiseAlpha = 1
const maximumNoiseAlpha = 10
...
export const acquisitionFromOptions = (
  options: TpeOptions
): Acquisition.Acquisition => Acquisition.resolve(options.acquisition)
...
export const validateOptions = (
  ...
  Effect.gen(function*() {
    ...
    yield* Effect.when(
      Effect.fail(
        new InvalidSamplerConfig({
          reason: `tpe sampler requires noiseAlpha to be between 0 and ${maximumNoiseAlpha}`,
          sampler: "tpe"
        })
      ),
