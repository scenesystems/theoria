...
import { type AcquisitionOption, defaultAcquisitionName, scoreJointAcquisition } from "../acquisition.js"
...
const multivariateTraceForGroup = (
  ...
  return Match.value(settings.multivariate).pipe(
...
export const suggestGroup = (
  ...
  return Effect.gen(function*() {
    ...
    const scoredCandidates = yield* Effect.forEach(
      indices,
      (index) => groupCandidateAtIndex(independentTraces, multivariateTrace, groupSplit, index, acquisition)
    )
    const candidateConfigs = Arr.map(scoredCandidates, (candidate) => candidate.config)
    const scores = Arr.map(scoredCandidates, (candidate) => candidate.score)
