...
const weightedMeanCost = (
  weightedSamplesInput: Iterable<{ readonly weight: number; readonly cost: number }>
): Option.Option<number> => {
  const weightedSamples = Arr.fromIterable(weightedSamplesInput)
...
const primitiveSimilarity = (left: unknown, right: unknown): number =>
  Match.value(Equal.equals(left, right)).pipe(
    Match.when(true, () => 1),
    Match.orElse(() => backgroundSimilarity)
  )
