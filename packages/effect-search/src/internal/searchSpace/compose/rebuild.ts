...
import { Condition, type Parameter, type SearchSpace as SearchSpaceType } from "../../../SearchSpace.js"
import { make, makeConditional } from "../compile/public.js"
...
const emptyDiscriminants = () => Arr.empty<string>()
...
const nextDiscriminantsForPath = (
  ...
  return Arr.reduce(
    ...
    (accumulator, discriminant) =>
      Match.value(Arr.contains(accumulator, discriminant)).pipe(
        Match.when(true, () => accumulator),
        Match.orElse(() => Arr.append(accumulator, discriminant))
      )
  )
}
