...
import { knownParameterNames, parameterByName } from "./parameters.js"
...
const unknownProjectionNames = (space: SearchSpace, namesInput: Iterable<string>) => {
...
const validateProjectionNames = (
  ...
  return Effect.gen(function*() {
    const requested = Arr.dedupe(names)
  ...
  return Effect.forEach(names, (name) => dependencyDimensionsForParameter(operation, space, name)).pipe(
    Effect.map((dependencySets) =>
      Arr.reduce(
        ...
        (accumulator, dependencies) => Arr.dedupe(Arr.appendAll(accumulator, dependencies))
      )
    )
  )
}
...
const expandDescendantClosure = (space: SearchSpace, omittedNamesInput: Iterable<string>) => {
  const omittedNames = Arr.fromIterable(omittedNamesInput)
  return Arr.reduce(
    space.params,
    ...
    (accumulator, parameter) =>
      Match.value(isDescendantOfOmitted(parameter, accumulator)).pipe(
        Match.when(true, () => Arr.dedupe(Arr.append(accumulator, parameter.name))),
        Match.orElse(() => accumulator)
      )
  )
}

const descendantClosure = (space: SearchSpace, omittedNamesInput: Iterable<string>): Names => {
  const omittedNames = Arr.fromIterable(omittedNamesInput)
