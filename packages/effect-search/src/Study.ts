...
import type { ArtifactStorageError, SearchError } from "./SearchError.js"
...
export class ObjectiveReport extends Schema.Class<ObjectiveReport>("effect-search/Study/ObjectiveReport")({
  value: Schema.Union(Schema.Number, Schema.Array(Schema.Number)),
  cost: Schema.optional(Schema.Number)
}) {}

/** Value accepted from objective callbacks. @since 0.7.0 @category schemas */
export const ObjectiveResult = Schema.Union(Schema.Number, Schema.Array(Schema.Number), ObjectiveReport)
...
export const events: <Space extends SearchSpace.SearchSpace>(study: Study<Space>) => Stream.Stream<StudyEvent.Event> = (
  study
...
export function snapshot<Config>(value: Result<Config>): Effect.Effect<StudySnapshot.Snapshot>
...
/** Storage requirement retained by durable resume operations. @since 0.7.0 @category type-level */
export type Storage = StudyStorage
/** Persistence failures produced by manual cancellation publication. @since 0.7.0 @category type-level */
export type StorageError = ArtifactStorageError
