...
import { type Value, Value as ObjectiveValue } from "./Objective.js"
...
const CompletedState = Schema.Struct({
  ...StudyTrial.Completed(ObjectiveValue).fields,
...
/** Constructs a search trial without inventing a schema for its generic configuration. @since 0.7.0 @category constructors */
export const make = <Config>(fields: StudyTrial.Trial<Config, State>): Trial<Config> => fields
...
export const makeRunning = <Config>(trialNumber: number, config: Config, startedAt: number): Trial<Config> =>
  make({ trialNumber, config, state: Running({ startedAt }) })
...
export const fail: {
  ...
  <Config>(self: Trial<Config>, error: TrialError, now: number) =>
    make({ ...self, state: Failed({ error, duration: durationFromState(self.state, now) }) })
)
...
export const prune: {
  ...
  <Config>(self: Trial<Config>, step: number, reason: string, policy: string, now: number) =>
    make({ ...self, state: Pruned({ step, reason, policy, duration: durationFromState(self.state, now) }) })
)

/** Records terminal cancellation. @since 0.7.0 @category combinators */
export const cancel = <Config>(self: Trial<Config>): Trial<Config> => make({ ...self, state: Cancelled({}) })
