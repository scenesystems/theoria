...
import { Envelope } from "./Artifact.js"
...
const writeFailure = (cause: Journal.Failure): ArtifactStorageError =>
  new ArtifactStorageError({ operation: "write", path: cause.path, detail: cause.detail })

const readFailure = (cause: Journal.Failure): ArtifactStorageError =>
...
export const layerFileSystem = (
  ...
  Layer.effect(
    ArtifactSink,
    Journal.make(Envelope, directory, fileName).pipe(
...
export const read = (
  ...
  Journal.read(Envelope, path).pipe(Stream.mapError(readFailure))
