...
const DEFAULT_FILE_NAME = "envelopes.jsonl"
const COMPONENT = Arr.of("Study")
const Trials = Schema.Array(StudySnapshot.Trial)
const VERSION: StudyArtifact.Version = "artifact-envelope/v1"
...
export const options = (directory: string, fileName = DEFAULT_FILE_NAME): Options =>
  new Options({ directory, fileName })
...
export class StudyStorage extends Effect.Tag("effect-search/StudyStorage")<
  StudyStorage,
  {
    ...
    readonly writeSnapshot: (snapshot: StudySnapshot.Snapshot) => Effect.Effect<void, ArtifactStorageError>
    readonly loadSnapshot: () => Effect.Effect<Option.Option<StudySnapshot.Snapshot>, ArtifactStorageError>
...
export const writeIfAvailable = (snapshot: StudySnapshot.Snapshot): Effect.Effect<void, ArtifactStorageError> =>
