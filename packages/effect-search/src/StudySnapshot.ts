...
export class Snapshot extends Schema.Class<Snapshot>("effect-search/StudySnapshot")(
  Fields
) {}
...
const materialize = (metadata: Metadata, trials: Iterable<Trial>, completedCount: number): Snapshot => {
...
export const toTrial = <Config>(trial: Trial, config: Config): SearchTrial.Trial<Config> =>
  SearchTrial.make({ ...trial, config })

/** Builds a snapshot and derives all counters. @since 0.7.0 @category constructors */
export const make = <Config>(trials: Iterable<SearchTrial.Trial<Config>>, metadata: Metadata): Snapshot => {
...
/** Decodes a snapshot and recomputes derived diagnostics. @since 0.7.0 @category decoders */
export const decode = (input: unknown) =>
  Schema.decodeUnknown(Snapshot)(input).pipe(
...
): Effect.Effect<Snapshot, InvalidStudyConfig> =>
  Effect.gen(function*() {
...
export const restore = <Space extends SearchSpace.SearchSpace>(
  ...
  Effect.gen(function*() {
    const persisted = yield* Schema.decodeUnknown(Snapshot)(snapshot).pipe(
    ...
    yield* Effect.when(
      ...
      () => Bool.not(Equal.equals(decoded.objectiveSpec, objectiveSpec))
    )
    yield* Effect.when(
    ...
    yield* Effect.when(
      ...
      () => Bool.not(Equal.equals(decoded.samplerKind, sampler.kind))
    )
