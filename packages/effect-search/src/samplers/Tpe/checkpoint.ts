 ...
 * @since 0.1.0
 */
import { Boolean, Effect, Equal, Match } from "effect"
...
export const restoreCheckpoint = (
  ...
  Match.value(checkpoint).pipe(
    Match.tag("Tpe", ({ seed: checkpointSeed, nStartupTrials, nEiCandidates }) =>
      Match.value(
        Boolean.and(
          Boolean.and(Equal.equals(seed, checkpointSeed), Equal.equals(startupTrials, nStartupTrials)),
          Equal.equals(nCandidates, nEiCandidates)
        )
      ).pipe(
