...
import { Direction, type Direction as DirectionValue, minimize, orDefault } from "./Direction.js"
...
export const directionAt = (objective: Objective, index: number): Option.Option<DirectionValue> =>
  match({
    Single: ({ direction }) =>
      Match.value(index).pipe(
        Match.when(0, () => Option.some(direction)),
        Match.orElse(() => Option.none<DirectionValue>())
      ),
...
export const single = (direction: DirectionValue = minimize): Objective => Single({ direction })

/** Copies directions into a multi-objective specification. @since 0.1.0 @category constructors */
export const multi = (directions: Iterable<DirectionValue>): Objective =>
