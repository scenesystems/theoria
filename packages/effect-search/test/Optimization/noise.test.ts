  ...
  it.effect("aggregates k evaluations into one completed trial with mean and variance metadata", () =>
    Effect.gen(function*() {
      ...
      const variances = Arr.map(
        completed,
        (state) => Option.fromNullable(state.variance).pipe(Option.getOrElse(() => Num.negate(1)))
      )
  ...
  it.effect("averages asymmetric vector samples by coordinate and sums only reported costs", () =>
    Effect.gen(function*() {
      const reports = Arr.make(
        new Optimization.ObjectiveReport({ value: Arr.make(1, 10) }),
        new Optimization.ObjectiveReport({ value: Arr.make(3, 16), cost: 0 }),
        new Optimization.ObjectiveReport({ value: Arr.make(8, 25), cost: 5 })
      )
