...
import { Array as Arr, Chunk, Effect, Match, Number as Num, Option, Stream } from "effect"
...
describe("sampler extensibility debt-prevention gate", () => {
  it.effect("runs optimize and optimizeStream through sampler interface without study-internal specialization", () =>
    Effect.gen(function*() {
      ...
      const expectedSlots = Arr.map(Arr.fromIterable(result.trials), (trial) => (trial.trialNumber + 5) % 17)
      ...
      expect(Arr.filter(tags, (tag) => tag === "TrialStarted")).toHaveLength(12)
      expect(Arr.filter(tags, (tag) => tag === "TrialCompleted")).toHaveLength(12)
  ...
  it.effect("supports snapshot/resume with sampler-owned checkpoint contracts", () =>
    Effect.gen(function*() {
      ...
      expect(Arr.map(Arr.fromIterable(resumedOption.value.trials), (trial) => trial.trialNumber)).toEqual([
        0,
        1,
        2,
        3,
