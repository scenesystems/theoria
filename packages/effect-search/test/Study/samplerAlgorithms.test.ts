...
describe("integration advanced samplers", () => {
  it.effect("runs end-to-end optimization with CMA-ES and GP-BO samplers", () =>
    Effect.gen(function*() {
      ...
      expect(Arr.length(Arr.fromIterable(cmaSingle.value.trials))).toBe(18)
      expect(Arr.length(Arr.fromIterable(gpSingle.value.trials))).toBe(18)
    }))
  ...
  it.effect("preserves sampler checkpoint continuity across snapshot and resume", () =>
    Effect.gen(function*() {
      ...
      expect(Arr.length(Arr.fromIterable(resumedSingle.value.trials))).toBe(14)
