...
import { Array as Arr, Data, Duration, Effect, Layer, Match, Option, Predicate, Ref, Schema } from "effect"
...
describe("Study scoped execution", () => {
  ...
  it.live("persists a restorable interruption snapshot through ambient optional storage", () =>
    Effect.gen(function*() {
      const checkpointCallsRef = yield* Ref.make(0)
      const interruptionSnapshotRef = yield* Ref.make<Option.Option<StudySnapshot.StudySnapshot>>(Option.none())
