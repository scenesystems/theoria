...
const traceFor = (result: Study.SingleObjectiveResult) =>
      ...
      Effect.forEach(
        ...
        (scenario) =>
          Effect.gen(function*() {
            ...
            const tpeTrials = Arr.fromIterable(tpeOption.value.trials)
            const randomTrials = Arr.fromIterable(randomOption.value.trials)
