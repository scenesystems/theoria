  ...
  it.effect("supports deterministic ask -> tell accumulation and returns Result contracts", () =>
    Effect.scoped(
      Effect.gen(function*() {
        ...
        expect(Arr.map(Arr.fromIterable(result.trials), (trial) => trial.trialNumber)).toEqual([0, 1, 2])

        const completedValues = Arr.flatMap(Arr.fromIterable(result.trials), (trial) =>
