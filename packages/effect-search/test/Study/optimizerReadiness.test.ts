...
import { Array as Arr, Effect, Match, Number as Num, Option, Ref, Schema } from "effect"
...
const asMultiObjective = (result: Study.Result) =>
...
describe("optimizer readiness pruning regression", () => {
  ...
  it.live("BootstrapFewShot readiness keeps deterministic random/grid baselines with bounded concurrency", () =>
    Effect.gen(function*() {
      ...
      expect(Arr.map(Arr.fromIterable(leftOption.value.trials), (trial) => trial.config)).toEqual(
        Arr.map(Arr.fromIterable(rightOption.value.trials), (trial) => trial.config)
      )
  ...
  it.effect("MIPROv2 readiness keeps categorical-coupled multivariate TPE deterministic and competitive", () =>
    Effect.gen(function*() {
      ...
      expect(Arr.map(Arr.fromIterable(tpeLeftOption.value.trials), (trial) => trial.config)).toEqual(
        Arr.map(Arr.fromIterable(tpeRightOption.value.trials), (trial) => trial.config)
      )
