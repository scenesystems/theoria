...
import { Array as Arr, Boolean, Effect, Equal, Number as Num, Tuple } from "effect"
import type { Schema } from "effect"

import * as Sampler from "../../src/Sampler/index.js"

const countSelections = (samples: Schema.Array$<typeof Schema.Number>["Type"], index: number): number =>
  Arr.reduce(
    samples,
    0,
    (count, selected) =>
      Boolean.match(Equal.equals(selected, index), {
        onFalse: () => count,
...
describe("Sampler weighted utilities", () => {
  ...
  it.effect("tracks weighted probabilities on a deterministic stream", () =>
    Effect.sync(() => {
      ...
      const withinTolerance = Arr.every(weights, (weight) => {
        const observed = Numeric.unsafeDivide(countSelections(draws, weight.index), sampleCount)
        const expected = Numeric.unsafeDivide(weight.weight, totalWeight)
  ...
  it.effect("normalizes finite weights before totals can overflow", () =>
    Effect.sync(() => {
      const weights = Arr.make(
        ...
        { index: 2, weight: Numeric.unsafeDivide(Number.MAX_VALUE, 2) }
      )
      const draws = Sampler.sampleWeightedIndices(weights, 10000, 42)
  ...
  it.effect("excludes non-finite and non-positive weights", () =>
    Effect.sync(() => {
      const weights = Arr.make(
        { index: -1, weight: Number.POSITIVE_INFINITY },
  ...
  it.effect("uses successive deterministic draws for pairs and optionally enforces distinct indices", () =>
    Effect.sync(() => {
      ...
      expect(Tuple.getFirst(distinctPair)).not.toBe(Tuple.getSecond(distinctPair))
      expect(Tuple.getFirst(zeroWeightDistinctPair)).not.toBe(Tuple.getSecond(zeroWeightDistinctPair))
    }))
})
