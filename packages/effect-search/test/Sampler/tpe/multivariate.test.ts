...
const valueAt = (values: GaussianVector, index: number): number =>
...
describe("multivariate gaussian foundations", () => {
  ...
  it.effect("samples deterministic coordinates from a diagonal gaussian kernel", () =>
    Effect.sync(() => {
      ...
      expect(valueAt(sample, 0)).toBeCloseTo(0.2436896868910798, 12)
      expect(valueAt(sample, 1)).toBeCloseTo(-0.4873793737821596, 12)
    }))
  ...
  it.effect("samples and scores a diagonal gaussian mixture", () =>
    Effect.sync(() => {
      ...
      expect(sample).toHaveLength(2)
      expect(valueAt(sample, 0)).toBeCloseTo(0.16146795335924322, 12)
      expect(valueAt(sample, 1)).toBeCloseTo(-0.1951198974583919, 12)
