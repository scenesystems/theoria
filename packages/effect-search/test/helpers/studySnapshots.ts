...
export const snapshotMultiObjectiveResult = (
  result: Study.Result
): Option.Option<Study.MultiObjectiveResult> =>
  Match.value(result).pipe(
    Match.tag("MultiObjective", (multi) => Option.some(multi)),
    Match.orElse(() => Option.none())
  )

export const snapshotConfigTrace = (result: Study.SingleObjectiveResult) =>
  Effect.forEach(result.trials, (trial) => decodeSnapshotConfig(trial.config))

export const snapshotValueTrace = (result: Study.SingleObjectiveResult): Array<number> =>
...
export const multiObjectiveConfigTrace = (result: Study.MultiObjectiveResult) =>
  Effect.forEach(result.trials, (trial) => decodePromptCategoricalConfig(trial.config))

export const multiObjectiveValueTrace = (result: Study.MultiObjectiveResult) =>
  Arr.flatMap(Arr.fromIterable(result.trials), (trial) =>
    Match.value(trial.state).pipe(
...
export const paretoObjectiveValueTrace = (result: Study.MultiObjectiveResult) =>
...
const projectSnapshotEvent = (event: StudyEvent.StudyEvent): string =>
...
const snapshotEventTrialNumber = (event: StudyEvent.StudyEvent): Option.Option<number> =>
  Match.value(event).pipe(
...
export const resumeSnapshotWithEvents = (options: Study.ResumeOptions) =>
  Effect.gen(function*() {
...
export const snapshotEventTrace = (events: Iterable<StudyEvent.StudyEvent>): Array<string> =>
  Arr.map(Arr.fromIterable(events), projectSnapshotEvent)

export const collectSnapshotEvents = (options: Study.Options) => Stream.runCollect(Study.optimizeStream(options))
