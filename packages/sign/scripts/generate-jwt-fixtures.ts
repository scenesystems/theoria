...
import { Array as Arr, Effect, Encoding, Number as N, Schema, String as Str } from "effect"
...
const successful = (operation: string, command: Command.Command) =>
  command.pipe(
    Command.exitCode,
    Effect.filterOrFail((code) => N.Equivalence(code, 0), () => new FixtureGenerationFailed({ operation }))
  )
...
const program = Effect.gen(function*() {
  ...
  const generated = yield* Effect.forEach(corpora, (corpus) =>
    Effect.gen(function*() {
      ...
      const fixture = yield* Schema.decodeUnknown(JwtFixture)({
        generator,
