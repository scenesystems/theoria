...
import { equalBytes, toHex, utf8ToBytes } from "@scenesystems/sign"
...
describe("utf8ToBytes", () => {
  it.effect("preserves Unicode scalars, BOM, combining marks, and NUL without normalization", () =>
    Effect.gen(function*() {
      expect(yield* Schema.encode(Schema.Uint8Array)(utf8ToBytes("Aé🔑e\u0301\0"))).toEqual(
        Arr.make(0x41, 0xc3, 0xa9, 0xf0, 0x9f, 0x94, 0x91, 0x65, 0xcc, 0x81, 0x00)
      )
      expect(yield* Schema.encode(Schema.Uint8Array)(utf8ToBytes("\ufeffA"))).toEqual(
        Arr.make(0xef, 0xbb, 0xbf, 0x41)
      )
      expect(yield* Schema.encode(Schema.Uint8Array)(utf8ToBytes("🔑"))).toEqual(
        Arr.make(0xf0, 0x9f, 0x94, 0x91)
      )
      expect(yield* Schema.encode(Schema.Uint8Array)(utf8ToBytes(""))).toEqual(Arr.empty())
    }))
  ...
  it.effect("replaces lone UTF-16 surrogates and returns a fresh byte array", () =>
    Effect.gen(function*() {
      ...
      const first = utf8ToBytes("fresh")
      const second = utf8ToBytes("fresh")
      expect(first).not.toBe(second)
    }))
})

describe("toHex", () => {
  it.effect("retains zero padding, byte order, and lowercase digits", () =>
    Effect.forEach(
      Arr.make(
        Tuple.make(Arr.make(0x0a, 0xff, 0x00, 0x7f), "0aff007f"),
        Tuple.make(Arr.of(0xab), "ab"),
        Tuple.make(Arr.empty<number>(), ""),
        Tuple.make(Arr.replicate(0xde, 32), "dededededededededededededededededededededededededededededededede")
      ),
      ([bytes, expected]) =>
        Effect.gen(function*() {
          expect(toHex(yield* Schema.decode(Schema.Uint8Array)(bytes))).toBe(expected)
        })
    ))
})
    ...
    Effect.forEach(
      ...
      ([left, right, expected]) =>
        Effect.gen(function*() {
          ...
          expect(equalBytes(a, b)).toBe(expected)
          expect(equalBytes(b, a)).toBe(expected)
        })
    ))
})
