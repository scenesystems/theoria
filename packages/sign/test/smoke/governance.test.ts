/**
 * Structural governance tests for @scenesystems/sign.
 *
 * Enforces three invariants:
 * 1. File size discipline — src files stay under 240 LOC
 * 2. Internal boundary — no src files outside internal/ import from internal/
 * 3. Export governance — package.json exports match expected surface
 *
 * Sign's internal/ contains only keyEncoding.ts, which is referenced
 * via JSDoc but not imported by any src file. The boundary test
 * enforces this remains true — no leakage of internal encoding
 * into the public module graph.
 */
...
const INTERNAL_IMPORT_ALLOWED_PREFIXES: ReadonlyArray<string> = [
  "src/algorithms/"
]
...
const hasAllowedInternalImport = (relativePath: string): boolean =>
  INTERNAL_IMPORT_ALLOWED_PREFIXES.some((allowed) => relativePath.startsWith(allowed) || relativePath === allowed)
...
const internalBoundaryViolations: Effect.Effect<Array<string>, never, FileSystem.FileSystem | Path.Path> = Effect.gen(
  function*() {
    ...
    const findings = yield* Effect.forEach(files, (file) =>
      fileSystem.readFileString(file.absolute).pipe(
        Effect.orDie,
        Effect.map((content) => INTERNAL_IMPORT_PATTERN.test(content) && !hasAllowedInternalImport(file.relative)),
