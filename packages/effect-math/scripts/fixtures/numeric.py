...
def _elementary_cases() -> list[dict[str, Any]]:
    ...
    # Exercise exact large-angle reduction throughout the binary64 exponent
    # range, not just powers of ten. Adjacent floats around large quadrant
    # boundaries detect an incorrect quotient/sign or lost binary input bits.
    large_angles = list(numpy.ldexp(numpy.linspace(1.03125, 1.96875, 64), numpy.linspace(21, 1023, 64, dtype=int)))
    quadrant_boundaries = [1_647_099.0, 1e23] + [float(numpy.ldexp(numpy.pi / 2, e)) for e in [21, 32, 48, 52]]
    large_angles += [float(numpy.nextafter(x, direction)) for x in quadrant_boundaries for direction in [-numpy.inf, numpy.inf]]
    large_angles += [-x for x in large_angles]
    ...
    inputs = {
        ...
        "sin": (numpy.sin, angles + signed + large_angles),
        "cos": (numpy.cos, angles + signed + large_angles),
