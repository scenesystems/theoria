...
describe("LinearAlgebra / add", () => {
  it.effect("adds two vectors elementwise", () =>
    Effect.gen(function*() {
      const result = add(Chunk.make(1, 2, 3), Chunk.make(4, 5, 6))
      expect(Equal.equals(result, Chunk.make(5, 7, 9))).toBe(true)
      expect(Equal.equals(add(Chunk.make(4, 5, 6))(Chunk.make(1, 2, 3)), result)).toBe(true)
    }))
})

describe("LinearAlgebra / scale", () => {
  it.effect("scales vector by scalar", () =>
    Effect.gen(function*() {
      const result = scale(Chunk.make(1, 2, 3), 2)
      expect(Equal.equals(result, Chunk.make(2, 4, 6))).toBe(true)
      expect(Equal.equals(scale(2)(Chunk.make(1, 2, 3)), result)).toBe(true)
    }))
})
