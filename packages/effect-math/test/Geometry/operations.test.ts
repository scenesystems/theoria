...
describe("Geometry / euclideanDistance", () => {
  it.effect("computes distance between [0,0] and [3,4]", () =>
    Effect.gen(function*() {
      ...
      expect(euclideanDistance(b)(a)).toStrictEqual(5)
    }))
...
describe("Geometry / centroidValidated", () => {
  it.effect("computes a centroid directly from decoded points", () =>
    Effect.gen(function*() {
      const result = centroid(Chunk.make(Chunk.make(0, 0), Chunk.make(4, 0), Chunk.make(2, 6)))
      expect(Equal.equals(result, Chunk.make(2, 2))).toBe(true)
    }))
