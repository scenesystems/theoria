 ...
 * @category internal
 */
import { BigDecimal, Boolean, Chunk, Iterable, Match, Number, Option, Predicate, Schema, Tuple } from "effect"
...
const LN_2_HIGH = 6.93147180369123816490e-1
const LN_2_LOW = 1.90821492927058770002e-10
const LOG_EVEN_COEFFICIENTS = Chunk.make(1.531383769920937332e-1, 2.222219843214978396e-1, 3.999999999940941908e-1)
const LOG_ODD_COEFFICIENTS = Chunk.make(
  1.479819860511658591e-1,
  1.818357216161805012e-1,
  2.857142874366239149e-1,
  6.666666666666735130e-1
)
const EXP_COEFFICIENTS = Chunk.make(
...
const integer = Predicate.or(
  ...
  Predicate.and(finite, (value: number) => Number.greaterThan(Binary.abs(value), MAX_SAFE_INTEGER))
)
...
class TrigSeriesState extends Schema.Class<TrigSeriesState>("TrigSeriesState")({
  cosine: Schema.Number,
  cosineTerm: Schema.Number,
  index: Schema.Number,
  sine: Schema.Number,
  sineTerm: Schema.Number
}) {}

class DecimalSeriesState extends Schema.Class<DecimalSeriesState>("DecimalSeriesState")({
  denominator: Schema.Number,
  term: Schema.BigDecimalFromSelf,
  total: Schema.BigDecimalFromSelf
}) {}
...
const logarithmFinitePositive = (value: number): number => {
  ...
  const remainder = Number.sum(
    Number.multiply(w, horner(LOG_EVEN_COEFFICIENTS, w)),
    Number.multiply(z, horner(LOG_ODD_COEFFICIENTS, w))
  )
  const halfSquare = Number.multiply(0.5, Number.multiply(f, f))
  return Number.subtract(
    Number.multiply(exponent, LN_2_HIGH),
    Number.subtract(
      Number.subtract(
        halfSquare,
        Number.sum(Number.multiply(s, Number.sum(halfSquare, remainder)), Number.multiply(exponent, LN_2_LOW))
      ),
      f
    )
  )
...
const logarithmDecimalFinitePositive = (value: number): BigDecimal.BigDecimal => {
  ...
  const normalized = Boolean.match(Number.greaterThan(rawMantissa, SQRT_2), {
    onTrue: () =>
  ...
  const series = Iterable.reduce(
    Iterable.take(Iterable.makeBy(() => 0), LOG_TERMS),
  ...
  return BigDecimal.sum(
    ...
    BigDecimal.multiply(BigDecimal.unsafeFromNumber(normalizedExponent), LN_2_DECIMAL)
  )
}
...
export const logStrict = (value: number): number =>
  Match.value(value).pipe(
    ...
    Match.orElse((value) => {
      ...
      return Number.sum(Number.multiply(2, total), Number.multiply(Number.sum(dyadic.exponent, 52), LN_2))
    })
  )

const alternatingSeries = (value: number): number => {
  const [, , total] = Iterable.reduce(
    Iterable.take(Iterable.makeBy(Number.increment), SMALL_SERIES_TERMS),
...
const exponentialReduced = (value: number): BigDecimal.BigDecimal => {
  const exponent = Binary.floor(Number.sum(Number.unsafeDivide(value, LN_2), 0.5))
  const reduced = BigDecimal.subtract(
    Binary.exactDecimal(value),
    BigDecimal.multiply(BigDecimal.unsafeFromNumber(exponent), LN_2_DECIMAL)
  )
  const series = Iterable.reduce(
    Iterable.take(Iterable.makeBy(() => 0), EXP_TERMS),
...
const EULER = BigDecimal.unsafeToNumber(exponentialReduced(1))

const exponentialFinite = (value: number): number => {
  const exponent = Number.round(Number.multiply(value, 1.44269504088896338700), 0)
  const high = Number.subtract(value, Number.multiply(exponent, LN_2_HIGH))
  const low = Number.multiply(exponent, LN_2_LOW)
  const reduced = Number.subtract(high, low)
  const square = Number.multiply(reduced, reduced)
  const correction = Number.subtract(reduced, Number.multiply(square, horner(EXP_COEFFICIENTS, square)))
...
export const exp = (value: number): number =>
  Match.value(value).pipe(
    ...
    Match.when((value) => Number.Equivalence(value, 1), () => EULER),
    Match.when(
...
export const log10 = (value: number): number =>
  Match.value(value).pipe(
    ...
    Match.when(Number.lessThan(0), () => Binary.notANumber),
    Match.orElse((value) =>
      BigDecimal.unsafeToNumber(BigDecimal.unsafeDivide(logarithmDecimalFinitePositive(value), LN_10_DECIMAL))
    )
  )
...
const PI_DECIMAL = BigDecimal.unsafeFromString(
  "3."
)
const TWO_PI_DECIMAL = BigDecimal.multiply(PI_DECIMAL, BigDecimal.make(2n, 0))
const HALF_PI_DECIMAL = BigDecimal.multiply(PI_DECIMAL, BigDecimal.make(5n, 1))
const QUARTER_PI_DECIMAL = BigDecimal.multiply(PI_DECIMAL, BigDecimal.make(25n, 2))
const THREE_QUARTER_PI_DECIMAL = BigDecimal.multiply(QUARTER_PI_DECIMAL, BigDecimal.make(3n, 0))

const reducedAngle = (value: number): BigDecimal.BigDecimal => {
  const remainder = BigDecimal.unsafeRemainder(Binary.exactDecimal(value), TWO_PI_DECIMAL)
  return Match.value(remainder).pipe(
    Match.when(
      (value) => BigDecimal.greaterThan(value, PI_DECIMAL),
      (value) => BigDecimal.subtract(value, TWO_PI_DECIMAL)
    ),
    Match.when(
      (value) => BigDecimal.lessThan(value, BigDecimal.negate(PI_DECIMAL)),
      (value) => BigDecimal.sum(value, TWO_PI_DECIMAL)
    ),
...
const trigSeries = (value: number) => {
  ...
  const result = Iterable.reduce(
    Iterable.take(Iterable.makeBy(() => 0), TRIG_TERMS),
...
const sinCosFinite = (value: number) => {
  ...
  return Match.value(principal).pipe(
    Match.when(BigDecimal.greaterThan(THREE_QUARTER_PI_DECIMAL), (angle) => {
      const [sine, cosine] = trigSeries(BigDecimal.unsafeToNumber(BigDecimal.subtract(PI_DECIMAL, angle)))
      return Tuple.make(sine, Number.negate(cosine))
    }),
    Match.when(BigDecimal.greaterThan(QUARTER_PI_DECIMAL), (angle) => {
      const [sine, cosine] = trigSeries(BigDecimal.unsafeToNumber(BigDecimal.subtract(HALF_PI_DECIMAL, angle)))
      return Tuple.make(cosine, sine)
    }),
    Match.when(BigDecimal.lessThan(BigDecimal.negate(THREE_QUARTER_PI_DECIMAL)), (angle) => {
      const [sine, cosine] = trigSeries(
        BigDecimal.unsafeToNumber(BigDecimal.subtract(BigDecimal.negate(PI_DECIMAL), angle))
      )
      return Tuple.make(sine, Number.negate(cosine))
    }),
    Match.when(BigDecimal.lessThan(BigDecimal.negate(QUARTER_PI_DECIMAL)), (angle) => {
      const [sine, cosine] = trigSeries(
        BigDecimal.unsafeToNumber(BigDecimal.subtract(BigDecimal.negate(HALF_PI_DECIMAL), angle))
      )
...
const atanSeries = (value: number): number => {
  const square = Number.multiply(value, value)
  const [, , total] = Iterable.reduce(
    Iterable.take(Iterable.makeBy(() => 0), ATAN_TERMS),
...
const atanPositive = (value: number): number =>
  Match.value(value).pipe(
    Match.when(Number.greaterThan(1), (value) => Number.subtract(HALF_PI, atanUnit(Number.unsafeDivide(1, value)))),
...
export const atan2 = (y: number, x: number): number =>
  Match.value(Tuple.make(y, x)).pipe(
    ...
    Match.when(([y, x]) =>
      ...
      withSign(
        Boolean.match(negativeSign(x), {
          onTrue: () => THREE_QUARTER_PI,
          onFalse: () => QUARTER_PI
        }),
        y
      )),
    Match.when(([y]) => Predicate.or(positiveInfinity, negativeInfinity)(y), ([y]) => withSign(HALF_PI, y)),
    Match.when(
    ...
    Match.when(([, x]) => zero(x), ([y]) => withSign(HALF_PI, y)),
    Match.orElse(([y, x]) => {
