...
const positiveTail = (x: number): number =>
  Boolean.match(Number.lessThan(x, ERF_SPLIT_LARGE), {
...
const erfRightNonBig = (x: number): number =>
  Boolean.match(Number.lessThan(x, TWO_POW_NEG_28), {
    onTrue: () => nearZeroApproximation(x),
    onFalse: () =>
      Boolean.match(Number.lessThan(x, ERF_SPLIT_SMALL), {
        onTrue: () => polynomialApproximation(x),
        onFalse: () =>
          Boolean.match(Number.lessThan(x, ERF_SPLIT_MIDDLE), {
        ...
        onFalse: () =>
          Boolean.match(Number.Equivalence(x, -Infinity), {
            ...
            onFalse: () => {
              ...
              const rightValue = Boolean.match(Number.lessThan(absoluteX, ERF_RIGHT_NON_BIG_THRESHOLD), {
