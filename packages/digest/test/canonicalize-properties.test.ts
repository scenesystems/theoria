...
import { CanonicalizationError, canonicalize } from "../src/index.js"
...
describe("canonicalize — generated data laws", () => {
  it.effect.prop("is invariant to record insertion order", [distinctEntries], ([entries]) =>
    Effect.gen(function*() {
      const forward = yield* canonicalize(Record.fromEntries(entries))
      const reverse = yield* canonicalize(Record.fromEntries(Arr.reverse(entries)))
  ...
  it.effect.prop(
    ...
    ([keys]) =>
      Effect.gen(function*() {
        ...
        expect(yield* canonicalize(Record.fromEntries(Arr.map(keys, (key) => Tuple.make(key, null))))).toBe(expected)
      }),
    { fastCheck: { numRuns: 200 } }
  )
  ...
  it.effect.prop(
    ...
    ([value]) =>
      Effect.gen(function*() {
        ...
        expect(yield* canonicalize(decoded)).toBe(first)
      }),
    { fastCheck: { numRuns: 200 } }
  )
      ...
      Effect.gen(function*() {
        ...
        yield* Effect.forEach(Arr.make(malformed, Record.singleton(malformed, true)), (input) =>
          Effect.gen(function*() {
            ...
            const diagnostic = yield* Schema.encode(Schema.parseJson(CanonicalizationError))(error)
