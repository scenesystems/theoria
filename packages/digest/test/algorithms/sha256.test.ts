...
describe("sha256 — NIST FIPS 180-4 vectors", () => {
  ...
  it.effect("produces correct hash for 'abc'", () =>
    Effect.gen(function*() {
      const result = yield* sha256(encodeFixtureUtf8("abc"))
      expectDigest(result, sha256Vectors.abc)
    }))

  it.effect("produces correct hash for 'hello'", () =>
    Effect.gen(function*() {
      const result = yield* sha256(encodeFixtureUtf8("hello"))
      expectDigest(result, sha256Vectors.hello)
    }))

  it.effect("produces correct hash for two-block 448-bit message", () =>
    Effect.gen(function*() {
      const input = encodeFixtureUtf8("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq")
...
describe("sha256 — output shape", () => {
  it.effect("output is always 32 bytes", () =>
    Effect.gen(function*() {
      ...
      const b = yield* sha256(encodeFixtureUtf8("hello"))
      const c = yield* sha256(encodeFixtureUtf8("a".repeat(10000)))
