...
export const mac = (
  ...
  key.length === 32
    ? Either.right(blake3(message, { key }))
    : Either.left(new InvalidKeyLength({ expected: 32, actual: key.length }))
...
export const deriveKey = (
  ...
  Number.isSafeInteger(length) && length >= 0
    ? Either.map(Utf8.encode(context), (encodedContext) => blake3(input, { context: encodedContext, dkLen: length }))
