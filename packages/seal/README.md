...
export const encryptForWire = (key: Uint8Array, plaintext: Uint8Array) =>
  Effect.gen(function* () {
    ...
    return Data.struct({ envelope, bytes })
  })
...
- `Cipher.InvalidKey`: wrong length or all-zero key, with `expected`, `received`, and `reason` fields.
- `Cipher.DecryptionFailed`: malformed base64url uses `invalid envelope encoding`; wrong keys, invalid lengths, and modified data use `authentication failed`. Carries `algorithm` and `reason`.
...
Use `Effect.catchTag` for expected failures. All failures use `Data.TaggedError`: they are in-process failures without a serialization requirement. A protocol owns its outward error representation. Backend exceptions never expose secrets in diagnostics. Treat all authentication failures the same way at a protocol boundary to avoid creating an oracle.
...
| `utf8ToBytes` / `utf8FromBytes`                                                             | Effect `Stream.encodeText` / `Stream.decodeText`                             |
| `equalBytes`                                                                                | `Schema.equivalence(Schema.Uint8ArrayFromSelf)` for public bytes             |

The decryption argument order changes: `unseal(key, envelope)` becomes `Envelope.decrypt(envelope, key)`.

`InvalidKey` and `DecryptionFailed` are no longer Schema codecs. Their tags and diagnostic fields remain usable with `Effect.catchTag`. Effect's byte equivalence is not a constant-time secret-comparison API.
...
The [tests](./test/) check independent Wycheproof and RFC 8452 known answers for all three algorithms, seeded byte-preservation laws, malformed data, key boundaries, entropy failure, lazy acquisition, interruption/finalization, and public envelope composition. Round trips supplement rather than replace conformance vectors.
