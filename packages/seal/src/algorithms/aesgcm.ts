...
export const aesgcmEncrypt = (
  ...
  Effect.gen(function*() {
    ...
    const aad = yield* validateAssociatedData("aes-256-gcm", Option.fromNullable(associatedData))
    return yield* Option.match(aad, {
      onNone: () => Effect.sync(() => managedNonce(gcm)(key).encrypt(plaintext)),
      onSome: (value) => Effect.sync(() => managedNonce(gcm)(key, value).encrypt(plaintext))
    })
  })
...
export const aesgcmDecrypt = (
  ...
  Effect.gen(function*() {
    ...
    return yield* Option.match(aad, {
      ...
      onSome: (value) =>
        Effect.try({
          try: () => managedNonce(gcm)(key, value).decrypt(ciphertext),
          catch: () =>
            new DecryptionFailed({
              algorithm: "aes-256-gcm",
              reason: "authentication failed"
