...
import { withModuleParamsDemos } from "@scenesystems/effect-dsp/contracts"
...
describe("destination-owned demonstrations", () => {
  it.effect("rejects incompatible labeled demos before changing any predictor", () =>
    Effect.gen(function*() {
      ...
      const failure = yield* Optimizer.labeledFewShot({ module: root, trainset: rows, k: 1 }).pipe(Effect.flip)
  ...
  it.effect("builds the automatic labeled baseline independently for each destination", () =>
    Effect.gen(function*() {
      const { root, child } = yield* makePipeline
      const mock = yield* MockLanguageModel.make(MockLanguageModel.fixed({ analysis: "Paris" }))
      ...
      expect((yield* Ref.get(root.params)).demos).toEqual(Arr.make(
        new Demo({ input: { question: "France" }, output: { answer: "Paris" } })
      ))
  ...
  it.effect("bootstraps stage traces and replays the heterogeneous child with automatic text mode", () =>
    Effect.gen(function*() {
      ...
      expect((yield* Ref.get(child.params)).demos).toEqual(Arr.make(
        new Demo({ input: { question: "France", context: "Cities" }, output: { analysis: "Paris" } })
      ))
      expect(
  ...
  it.effect("uses only destination-valid labels and stage demos in MIPRO phase one", () =>
    Effect.gen(function*() {
      const { root, child } = yield* makePipeline
      const stage = new Demo({ input: { question: "France", context: "Cities" }, output: { analysis: "Paris" } })
      yield* Ref.update(child.params, (params) => withModuleParamsDemos(params, Arr.make(stage)))
      const sets = yield* Optimizer.generateDemoCandidates({ module: root, trainset: rows, numCandidates: 4 })
      ...
      yield* Effect.forEach(childSet.candidates, (candidate) =>
        Effect.forEach(candidate.params.demos, child.signature.demoContract.decode))
    }))
  ...
  it.effect("runs default BootstrapRS with a destination-aware baseline and replays its stage demos", () =>
    Effect.gen(function*() {
      ...
      const teacher = yield* MockLanguageModel.make(MockLanguageModel.fixed({ analysis: "training-stage-marker" }))
  ...
  it.effect("restores every predictor after interrupting a teacher run", () =>
    Effect.gen(function*() {
      ...
      const fiber = yield* Optimizer.bootstrapFewShot({
        module: root,
        trainset: rows,
        metric: Metric.exactMatch("answer"),
        maxRounds: 1,
