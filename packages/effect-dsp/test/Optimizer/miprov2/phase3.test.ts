...
import { runPhase3Search } from "@scenesystems/effect-dsp/Optimizer"
...
describe("MIPROv2 Phase 3", () => {
  ...
  it.effect("preflights a nonzero incompatible child demo before mutating the module tree", () =>
    Effect.gen(function*() {
      ...
      const mock = yield* MockLanguageModel.make(MockLanguageModel.fixed({ fact: "Paris", answer: "Paris" }))
      const failure = yield* runPhase3Search({
        module: root,
        valset: trainset,
        metric: Metric.exactMatch("answer"),
        demoCandidates,
  ...
  it.effect("rejects ambiguous, unknown, and internally mismatched candidate identities", () =>
    Effect.gen(function*() {
      ...
      const ambiguous = yield* runPhase3Search({
        module,
        valset: trainset,
        metric: Metric.exactMatch("answer"),
      ...
      const mismatched = yield* runPhase3Search({
        module,
        valset: trainset,
        metric: Metric.exactMatch("answer"),
      ...
      const unknown = yield* runPhase3Search({
        module,
        valset: trainset,
        metric: Metric.exactMatch("answer"),
