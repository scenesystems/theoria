...
const report = new Evaluate.Report({
  overallScores: {
    accuracy: 0.8,
    faithfulness: 0.6
  },
...
describe("contracts/ObjectiveProjection", () => {
  it.effect("projects deterministic single-objective payloads with telemetry", () =>
    Effect.gen(function*() {
      const projectedA = yield* Contracts.ObjectiveProjection.fromReport({ report, mode: "single" })
      const projectedB = yield* Contracts.ObjectiveProjection.fromReport({ report, mode: "single" })
  ...
  it.effect("projects deterministic multi-objective vectors with schema round-trip", () =>
    Effect.gen(function*() {
      const projected = yield* Contracts.ObjectiveProjection.fromReport({
        report,
        mode: "multi",
        metricNames: ["faithfulness", "accuracy"]
      })
