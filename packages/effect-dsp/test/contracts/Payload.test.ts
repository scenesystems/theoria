...
import { decodePayload, encodePayload, Payload } from "../../src/contracts/Payload.js"
...
describe("schema-bound payloads", () => {
  it.effect("retains encoded structured fields and restores decoded signature values", () =>
    Effect.gen(function*() {
      ...
      const document = yield* encodePayload(Facts, value)
      const restored = yield* decodePayload(Facts, document)
  ...
  it.effect.prop("round-trips nested values including escaped text", {
    ...
    Effect.gen(function*() {
      ...
      const document = yield* encodePayload(Facts, value)
      expect(yield* decodePayload(Facts, document)).toEqual(value)
    }))
  ...
  it.effect("rejects lossy numeric JSON and malformed persisted documents", () =>
    Effect.gen(function*() {
      ...
      const failure = yield* encodePayload(schema, { score: infinity }).pipe(Effect.flip)
      ...
      const valid = yield* encodePayload(schema, { score: null })
      expect(yield* decodePayload(schema, valid)).toEqual({ score: null })
    }))
  ...
  it.effect("keeps schema service requirements through encoding and decoding", () =>
    Effect.gen(function*() {
      ...
      const document = yield* encodePayload(schema, "with context").pipe(Effect.provideService(Prefix, "required:"))
      expect(yield* Ref.get(decodes)).toBe(0)
      const restored = yield* decodePayload(schema, document).pipe(Effect.provideService(Prefix, "required:"))
  ...
  it.effect("preserves wire data without imposing bijective domain transformations", () =>
    Effect.gen(function*() {
      ...
      const document = yield* encodePayload(schema, "MiXeD")
      expect(document).toBe("\"MiXeD\"")
      expect(yield* decodePayload(schema, document)).toBe("mixed")
    }))
  ...
  it.effect("reports unsupported schema equivalence as a checked failure", () =>
    Effect.gen(function*() {
      ...
      const failure = yield* encodePayload(schema, "safe").pipe(Effect.flip)
      expect(failure._tag).toBe("ParseError")
    }))
})
