...
import { Optimization, Pareto, Sampler, SearchSpace } from "@scenesystems/effect-search"
  ...
  it.effect("runs deterministic seeded ask/tell through the public Study and Sampler modules", () =>
    Effect.scoped(
      Effect.gen(function*() {
        ...
        yield* Optimization.tell(handle, second.trialNumber, second.config.x)
        ...
        expect(Arr.some(observed, (event) => String.Equivalence(event._tag, "Completed"))).toBe(true)
