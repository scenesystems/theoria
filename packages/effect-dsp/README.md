...
Search primitives are not mirrored. Import studies, samplers, Pareto operations,
artifact envelopes, and deterministic seed operations directly:
...
export const optimize = Study.optimize
```
...
`Cache.Cache`, `Cache.Key`, `Cache.key`, `Cache.layer`, `Cache.layerMemory`,
`Cache.layerFileSystem`, and `Cache.layerSql` provide language-model result
memoization over effect-search cache backends.
...
`Demonstration`, `Example`, `Metric`, `Evaluate`, `EvaluationObjective`, `Trace`, `Cache`, `Payload`,
