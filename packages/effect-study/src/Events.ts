 ...
 * delivered after events emitted before that failure. Defects are propagated as
 * defects, never converted to typed failures. Ending or interrupting
...
export const streamFromEmitter = <Emitted, A, E, R>(
  ...
  Stream.unwrapScoped(
    Effect.gen(function*() {
      const mailbox = yield* Mailbox.make<Emitted, E>()
      yield* Effect.addFinalizer(() => mailbox.shutdown)
      yield* Effect.suspend(() => runWithEmitter((event) => mailbox.offer(event).pipe(Effect.asVoid))).pipe(
