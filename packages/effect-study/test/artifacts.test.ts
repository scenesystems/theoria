...
const Envelope = Artifacts.makeEnvelopeSchema(Producer, Payload)

describe("Artifacts", () => {
  it.effect("composes an independent producer and transformed payload without losing codec requirements", () =>
    Effect.gen(function*() {
      const runId = yield* Schema.decode(Artifacts.RunId)("01ARZ3NDEKTSV4RRFFQ69G5FAV")
      ...
      const value: typeof Envelope.Type = {
        ...
        lineage: {
          ...
          artifactId: new Artifacts.ArtifactId({ runId, sequence: 4 }),
          emittedAt
        },
  ...
  it.effect("specializes source origins without duplicating lineage fields", () =>
    Effect.gen(function*() {
      const ClosedSourceRef = Artifacts.makeSourceRefSchema(Schema.Literal("first-party"))
      const ClosedLineage = Artifacts.makeLineageSchema(ClosedSourceRef)
      const valid = yield* Schema.decodeUnknown(ClosedSourceRef)({
        origin: "first-party",
        domain: "study",
