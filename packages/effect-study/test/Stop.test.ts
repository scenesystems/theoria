...
it.effect("keeps Drain cooperative and exposes Interrupt through polling", () =>
  Effect.gen(function*() {
    const ref = yield* Stop.make
    const decisionReason = Stop.matchDecision({
      Continue: () => "continue",
      Stop: ({ reason }) => reason
    })
    expect(decisionReason(yield* Stop.heartbeat(ref, "Interrupt"))).toBe("continue")

    yield* Stop.request(ref, earliestInterrupt)

    expect(decisionReason(yield* Stop.heartbeat(ref, "Drain"))).toBe("continue")
    expect(decisionReason(yield* Stop.heartbeat(ref, "Interrupt"))).toBe("alpha")
