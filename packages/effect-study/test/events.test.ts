...
import * as Study from "../src/Events.js"
...
describe("Study.streamFromEmitter", () => {
  ...
  it.effect("propagates producer defects after buffered events", () =>
    Effect.gen(function*() {
      ...
      const exit = yield* Study.streamFromEmitter((emit: Study.EmitterSink<string>) =>
  ...
  it.effect("releases the producer when the consumer ends early", () =>
    Effect.gen(function*() {
      ...
      const events = yield* Study.streamFromEmitter((emit: Study.EmitterSink<string>) =>
