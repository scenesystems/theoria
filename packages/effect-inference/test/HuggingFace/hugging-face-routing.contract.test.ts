...
describe("HuggingFace/hugging-face-routing", () => {
  it.effect("keeps brokered-provider routing explicit above the shared Hugging Face family", () =>
    Effect.sync(() => {
  ...
  it.effect("keeps dedicated-endpoint identity explicit above the shared Hugging Face family", () =>
    Effect.sync(() => {
      ...
      expect(resolution.capabilities.usageReporting).toBe(false)
    }))
})
