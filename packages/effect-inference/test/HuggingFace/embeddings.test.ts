...
import { explicitProviderSelection, RouteSelectionPolicySchema } from "../../src/contracts/RouteSelectionPolicy.js"
import * as HuggingFace from "../../src/HuggingFace/index.js"

const endpoint = new HuggingFace.HuggingFaceEmbeddingOptions({
  model: "org/embedding-model",
  route: HuggingFace.makeHuggingFaceEndpointRoute({
...
const routed = new HuggingFace.HuggingFaceEmbeddingOptions({
  model: "org/embedding-model",
  route: HuggingFace.makeHuggingFaceRoutedRoute({
...
const mapping = {
  inferenceProviderMapping: {
    ...
    "hf-inference": { providerId: "org/embedding-model", status: "live", task: "sentence-similarity" }
  }
}
...
describe("HuggingFace native embeddings", () => {
  it.effect("embeds at the exact endpoint with bearer auth, scalar/batch bodies, and no discovery", () =>
    Effect.gen(function*() {
      ...
      yield* Effect.gen(function*() {
        ...
        Effect.provide(HuggingFace.HuggingFaceEndpointEmbeddings(endpoint)),
  ...
  it.effect("discovers once, uses the first mapping and restores indexed vectors to input order", () =>
    Effect.gen(function*() {
      ...
      yield* Effect.gen(function*() {
        ...
        Effect.provide(HuggingFace.HuggingFaceRoutedEmbeddings(routed)),
        Effect.provideService(HttpClient.HttpClient, client)
      )
      const seen = yield* Ref.get(requests)
  ...
  it.effect("coalesces discovery, permits waiter cancellation, expires mappings, and isolates model layers", () =>
    Effect.gen(function*() {
      ...
      const layer = HuggingFace.HuggingFaceRoutedEmbeddings(routed)
      yield* Effect.gen(function*() {
        const model = yield* EmbeddingModel.EmbeddingModel
      ...
      (contract) =>
        Effect.forEach(
          ...
          (token) =>
            Effect.gen(function*() {
              const requests = yield* Ref.make(Chunk.empty<HttpClientRequest.HttpClientRequest>())
              const options = new HuggingFace.HuggingFaceEmbeddingOptions({
                model: routed.model,
                route: HuggingFace.makeHuggingFaceRoutedRoute({
                  baseUrl: "https://gateway.example.test/hf/v1/",
                  authMethod: "hf-token",
                  selectionPolicy: explicitProviderSelection(contract.provider)
                }),
              ...
              yield* Effect.gen(function*() {
                ...
                Effect.provide(HuggingFace.HuggingFaceRoutedEmbeddings(options)),
                Effect.provideService(HttpClient.HttpClient, client)
              )
              const seen = yield* Ref.get(requests)
              const discovery = yield* Chunk.get(seen, 0)
  ...
  it.effect("fails unsupported policies, providers, missing mappings and wrong tasks without inference fallback", () => {
    const Scenario = Schema.Struct({
      policy: Schema.optional(RouteSelectionPolicySchema),
    ...
    const scenarios = Arr.make(
      ...
      Scenario.make({ policy: explicitProviderSelection("deepinfra"), mapping, tag: "MalformedInput" }),
      Scenario.make({
        mapping: {
          inferenceProviderMapping: {
      ...
      Scenario.make({
        mapping: {
          inferenceProviderMapping: {
            ...
            "hf-inference": Option.getOrThrow(Record.get(mapping.inferenceProviderMapping, "hf-inference"))
          }
        },
        tag: "MalformedInput"
      }),
        ...
        Effect.forEach(Arr.make("https://wrong.example/model", "/model"), (model) =>
          Effect.gen(function*() {
            ...
            yield* Effect.gen(function*() {
              ...
              Effect.provide(
                HuggingFace.HuggingFaceEndpointEmbeddings(
                  new HuggingFace.HuggingFaceEmbeddingOptions({ ...options, model })
                )
              ),
              Effect.provideService(HttpClient.HttpClient, client)
            )
