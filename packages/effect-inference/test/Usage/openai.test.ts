...
import * as Usage from "../../src/Usage/index.js"
...
describe("Usage/observeOpenAi", () => {
  it.effect("observes all five native Responses API counters", () =>
    Effect.scoped(Effect.gen(function*() {
      ...
      const decorated = Usage.observeOpenAi(
        nativeClient,
        (usage, raw) =>
  ...
  it.effect.each(Arr.make(1, 2))(
    ...
    (count) =>
      Effect.scoped(Effect.gen(function*() {
        ...
        const observed = yield* Ref.make(Arr.empty<AiResponse.Usage>())
        const nativeClient = yield* makeClient(sseHttpClient(encodeSse(events)))
        const decorated = Usage.observeOpenAi(nativeClient, (usage) => Ref.update(observed, Arr.append(usage)))
