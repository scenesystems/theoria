...
const OpenRouterUsageObservation = Schema.transform(
...
const decodeUsageObservation = Schema.decodeSync(OpenRouterUsageObservation)
...
const observeOptional = (
  ...
  usage.pipe(
    Option.match({
      ...
      onSome: (raw) => observe(decodeUsageObservation(raw).usage, Option.some(raw))
    })
  )
 ...
 *   const observed = Usage.observeOpenRouter(client, (usage) => Effect.log(usage))
 ...
 */
export const observeOpenRouter = (
