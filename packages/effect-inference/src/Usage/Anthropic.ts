 ...
 * @category schemas
 */
export const AnthropicUsageObservationSchema = Schema.Union(
 ...
 * @category models
 */
export type AnthropicUsageObservation = typeof AnthropicUsageObservationSchema.Type
 ...
 * @category models
 */
export const AnthropicUsageObservation = Data.taggedEnum<AnthropicUsageObservation>()
...
const observeStream = (
  ...
  observe: (observation: AnthropicUsageObservation) => Effect.Effect<void>
            ...
            Match.value(event).pipe(
              Match.when({ type: "message_start" }, (event) => {
                ...
                return Ref.set(state, usage).pipe(
                  Effect.zipRight(observe(AnthropicUsageObservation.MessageStart({
                    usage,
              ...
              Match.when({ type: "message_delta" }, (event) =>
                Ref.updateAndGet(
                  ...
                  Effect.flatMap((usage) =>
                    observe(AnthropicUsageObservation.MessageDelta({
                      usage,
                      raw: event.usage
                    }))
                  )
...
(options) =>
  createMessage(options).pipe(
    Effect.tap((message) =>
      observe(AnthropicUsageObservation.Response({
 ...
 *   const observed = Usage.observeAnthropic(client, (observation) => Effect.log(observation))
