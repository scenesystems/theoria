/**
 * Native Hugging Face feature-extraction transport and chat model references.
 *
 * @since 0.1.0
 */
...
import type { HuggingFaceEmbeddingOptions } from "../HuggingFace/endpoint.js"
...
const selectProvider = (mappings: typeof MappingArray.Type, policy: Option.Option<RouteSelectionPolicy>) =>
  Match.value(Option.getOrElse(policy, (): RouteSelectionPolicy => "auto")).pipe(
    Match.when("auto", () =>
...
/** @internal */
export const makeHuggingFaceEmbeddingLayer = (
  options: HuggingFaceEmbeddingOptions
...
/** Encodes chat-only router selection. Feature extraction uses Hub mappings. @internal */
export const makeHuggingFaceRoutedModelRef = (
  ...
  selectionSuffix(selectionPolicy).pipe(Option.match({
    onNone: () => modelRef,
    onSome: (suffix) => String.concat(modelRef, String.concat(":", suffix))
  }))
