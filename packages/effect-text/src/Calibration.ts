 ...
 * searches for better profile parameters with reproducible Effect studies.
 *
 * @since 0.5.0
 * @module
 */
import { Statistics } from "@scenesystems/effect-math"
import { Sampler, SearchSpace, Study, StudyEvent } from "@scenesystems/effect-search"
...
import { runFreshStudy, runResumedStudy } from "./internal/calibration/study.js"
...
const StudyEventLog = Schema.Array(StudyEvent.StudyEventSchema)

/**
 * Resumable study snapshot and ordered events from one optimization run.
 *
 * @since 0.5.0
 * @category schemas
 */
export const StudyArtifacts = Schema.Struct({
 ...
 * @category models
 */
export type StudyArtifacts = typeof StudyArtifacts.Type
...
/**
 * Reports that supplied study storage did not retain the generated snapshot.
 * The wire tag remains `CalibrationSnapshotMissing` for persisted failures.
 *
 * @since 0.5.0
 * @category errors
 */
export class SnapshotMissing extends Schema.TaggedError<SnapshotMissing>()("CalibrationSnapshotMissing", {
  /** Number of trials the storage still retained in its trial log. */
  trialLogLength: NonNegativeInt
}) {}

/**
 * Reports that supplied storage resolved to a multi-objective study result.
 * The wire tag remains `CalibrationStudyNotSingleObjective` for persisted failures.
 *
 * @since 0.5.0
 * @category errors
 */
export class StudyNotSingleObjective extends Schema.TaggedError<StudyNotSingleObjective>()(
...
/**
 * Runs a fresh or resumed Effect Search study and selects the profile with the
...
export const optimize = (options: OptimizeOptions) =>
  Effect.gen(function*() {
    ...
    const bestProfile = Profile.make({ name: "best", profile: study.studyResult.bestTrial.config })
