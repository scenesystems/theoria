...
export const executionTimeoutError = (): EntryExecutionError =>
  new EntryExecutionError({
    ...
    message: "Study execution timed out.",
    retryable: true
  })
...
const unexpectedExecutionFailure = ({
  ...
  Effect.logError("theoria study execution failed").pipe(
...
export const resolveStudyExecutionError = ({
  entryId,
  executionId,
  error,
  runToken
...
export const normalizeStudyExecutionError = ({
  entryId,
  executionId,
  error,
  runToken
  ...
  readonly error: unknown
  readonly runToken: string
}) => resolveStudyExecutionError({ entryId, executionId, error, runToken }).pipe(Effect.flatMap(Effect.fail))
