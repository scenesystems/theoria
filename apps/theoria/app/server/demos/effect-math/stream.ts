...
import { section, step, type StreamElement } from "../stream-element.js"
...
const runtimeSummarySection = (request: EffectMathStreamRequest): EvidenceSection => {
  const runPlan = snapshotEffectMathRunPlan(request.controls)
...
const projectionStepElements = (request: EffectMathStreamRequest) =>
  snapshotEffectMathRunPlan(request.controls).phases.flatMap((phase) =>
