...
export const programForWorkflow = (workflowRun: FrozenWorkflowRun) =>
  multiFileProgram([
    programFile("workflow/baseline.graph.ts", renderRecord(workflowRun.baseline.record, "baseline")),
    programFile("workflow/optimized.graph.ts", renderRecord(workflowRun.optimized.record, "optimized"))
  ])
