...
import { EntryRoute, HomePageRoute, PackageDocsRoute, PageRoute, WorkflowStudyRoute } from "./path.js"
...
export class WorkflowStudyPagePresentation extends Schema.TaggedClass<WorkflowStudyPagePresentation>()(
  "WorkflowStudyPagePresentation",
  {
    entry: EntryPresentation,
    route: WorkflowStudyRoute,
    metadata: PageMetadata
  }
) {}
...
export class PagePresentation {
  static metadata(route: PageRoute.Value): PageMetadata {
    return Match.value(route).pipe(
      ...
      Match.tag("WorkflowStudyRoute", (value) => PageMetadata.fromWorkflowStudyRoute(value)),
  ...
  static project(route: PageRoute.Value): PagePresentation.Value {
    return Match.value(route).pipe(
      ...
      Match.tag(
        "WorkflowStudyRoute",
        (value) =>
          WorkflowStudyPagePresentation.make({
            entry: EntryPresentation.fromEntryId(workflowEntryId),
            route: value,
