...
import { EvidencePlaneOverviewViewModel } from "./plane-presentation.js"
import type { EvidencePlaneOrder } from "./plane.js"
import { EvidenceMetric, EvidenceSectionStats, type EvidenceSectionViewModel } from "./section-presentation.js"

const formatDuration = (durationMs: number): string =>
  durationMs >= 1000 ? `${(durationMs / 1000).toFixed(1)}s` : `${Math.round(durationMs)} ms`

const aggregateSectionStats = (sections: ReadonlyArray<EvidenceSectionViewModel>) =>
  sections.reduce(
    (totals, section) =>
...
export const buildEvidencePlaneOverview = ({
  ...
  return EvidencePlaneOverviewViewModel.make({
    ...
    description: activeSectionKey === null
      ? complete
        ? summary ?? "Decisive results stay promoted while datasets and context remain legible as supporting evidence."
        : order === "live"
...
export const evidencePlaneEmptyFilterText = (): string =>
  "No evidence sections match the current lens. Expand the focus or change the view to bring them back into projection."
