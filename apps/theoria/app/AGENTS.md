...
| File                            | Role                                                                                                                                                                                                                                                                |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `app.ts`                        | `publicApp` (router + `indexingPolicy` + `xForwardedHeaders` + `securityHeaders`) and `AppLayer` (`ParticipantsLive`, `DocsManifestStoreLive`, `RuntimeInfoLive`, release-stage check)                                                                              |
...
| `config/docs-manifest-store.ts` | `DocsManifestStore` service: the docs manifest read from the `StaticStore` at startup                                                                                                                                                                               |
...
| File                       | Role                                                                                                                                                                                                    |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `runtime.ts`               | `appRuntime = Atom.runtime(BrowserLive)` — every atom that touches the browser runs here and asks for `BrowserWindow`, `BrowserDocument`, `KeyValueStore`, `Clipboard` or `HttpClient` as services      |
...
| `theme.ts`                 | `colorModePreferenceAtom` (`Atom.kvs` over the cookie `KeyValueStore`, key `colorModeCookieName`, tri-state system/light/dark), derived `colorModeAtom`, and the mount atom that applies the root class |
...
- **Mount-scoped element observation**: Values derived from a live DOM element (`ResizeObserver`, viewport width, rects, visibility, scroll measurements). These live in `web/atoms/element-observation.ts` as non-`keepAlive` atoms keyed by the element itself (`elementWidthAtom(element)` is an `Atom.family` over the `ElementSize` stream). `useElementWidth` gives each component an ephemeral writable element slot subscribed through `useAtom`. Its React 19 callback ref explicitly mounts the slot in the registry before writing during the commit phase; this preserves the initial element until React subscribes, including registries with microtask disposal. Ref cleanup clears only its own attachment and releases that mount. Both slot and observation use zero idle TTL so DOM state is released on unmount. Never key observations by string ids like `useId()` or any other pseudo-identity.
...
| Directory     | Role                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `primitives/` | **All** reusable building blocks: layout (`Stack`, `Cluster`, `Layer`, `Section`), typography (`SemanticText`, `SemanticContent`), code (`CodeBlock`, `code/HighlightedCode`), controls over Base UI (`TabGroup`/`TabBar`/`Tab`/`TabPanel` on `Tabs`, `ChoiceGroup` on `RadioGroup`, `ToggleSwitch` on `Switch`, `FieldGroup`/`FieldLabel`/`FieldDescription`/`TextAreaField` on `Field`, `ActionButton`, `ActionControl`), feedback (`Skeleton`, `InlineStatus`, `ChangedValue`), names in the line (`ParticipantName`, `PackageName`), chrome (`SiteHeader`, `SiteFooter`, `ThemeToggle`, `TheoriaLogo`, `WordmarkMorph` animated with Motion). Layout primitives take Base UI's `render` prop for the element they become. Every visual atom lives here — `home/` and `docs/` compose primitives, never define them. |
| `home/`       | Home page: `HomePage`, `HomeHero`, and the Imagined Place demo (`PlaceActs`, `PlaceArrive`, `PlaceComposition`, `PlaceStage`, `PlaceGhosts`, `PlaceBand`, `PlaceWalk`, `PlaceMarker`, `PlaceProposals`, `PlaceProposal`, `PlaceArrangement`, `PlaceSearchTrace`, `PlaceStrand`, `PlaceControls`, `PlaceStepCard`, `PlaceHowItsBuilt`, `PlaceProvenance`, `ContentId`; pure projections in `placeViewModel.ts`, `placeSteps.ts`, `placeProvenance.ts`, `placeLiveValues.ts`, `placeMarkerLabels.ts`, `placeReferences.ts`)                                                                                                                                                                                                                                                                                               |
...
- **Primitives are generic.** `ArtifactStage`, `CodeBlock`, `TabBar` know nothing about the home demo or the docs — they render whatever contracts give them.
