...
import { Effect, Equal, Number as Num, Predicate } from "effect"
...
describeOnStage("place band", (it) => {
  it.effect("sets every disc of the place in one row, in order, none touching", () =>
    Effect.gen(function*() {
      ...
      const edges = Arr.map(row.discs, (disc) => ({
        left: Num.subtract(disc.cx, disc.marker.radius),
        right: Num.sum(disc.cx, disc.marker.radius)
      }))
  ...
  it.effect("is a strip: its paper is no thicker than a quarter of the tallest disc, above and below", () =>
    Effect.gen(function*() {
      const { kept } = yield* onStage
      const row = bandRow(kept.projection)
      const tallest = Arr.reduce(kept.projection.markers, 0, (widest, marker) => Num.max(widest, marker.radius))
      expect(tallest).toBeGreaterThan(0)
      expect(row.cy).toBe(Num.unsafeDivide(row.height, 2))
      expect(row.height).toBeLessThanOrEqual(Num.multiplyAll([tallest, 2, 1.25]))
