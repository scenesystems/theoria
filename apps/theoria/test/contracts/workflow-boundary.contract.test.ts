...
const workflowScenarioContractPath = "app/contracts/study/workflow/scenario.ts"
...
describe("Theoria Workflow Contract Boundary", () => {
  it.effect("imports workflow authority from effect-inference without re-exporting the package-owned family", () =>
    Effect.gen(function*() {
      const source = yield* readProjectFile(appRootUrl, workflowScenarioContractPath)
      const parsed = parseTypeScript(workflowScenarioContractPath, source)
