...
**The repository retains Vitest 4 and Vite 8.** `@effect/vitest` 0.30.0
declares the narrower peer range `vitest ^3.2.0`; that metadata does not
authorize downgrading the repository's chosen runner. Verify behavior with
the retained toolchain without treating passing tests as upstream support
for the pairing. CI worker limits come from Effect's `Config.boolean` and
`Boolean.match`. The root `coverage` block had sat outside `test`, where
Vitest ignored it, and nothing ran coverage; it and `@vitest/coverage-v8`
were removed. `bunfig.toml`, which pointed `bun test` at an empty directory,
is gone; use `bun run test` to run the Effect Vitest suites. Empty runs fail
