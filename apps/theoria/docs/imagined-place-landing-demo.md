...
| Record  | digest, sign                            | `ContentDigest.fromSchema` over the canonical artifact; version 2 carries version 1's ID as `parent`; the author signs every version                                                                             | Version rows with BLAKE3-256 IDs, `Built from v1`, `You signed · key …`              |
| Arrange | effect-text, effect-math, effect-search | In the browser, seeded TPE (`Sampler.tpe({ seed: 42 })`, 36 trials) over a six-parameter meander; `linesWith` wraps the description per line; distances and spread score each trial                              | Discs settle while the trace advances; text narrows beside each disc                 |
