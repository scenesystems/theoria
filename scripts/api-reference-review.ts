...
const key = (value: { readonly package: string; readonly module: string }): string =>
...
const program = Effect.gen(function*() {
  ...
  if (accept) {
    ...
    const record = yield* Schema.decodeUnknown(ReviewRecord)({
      ...
      duplicateAllowlist: Arr.filter(
        currentRecord.duplicateAllowlist,
        (allowed) => Arr.some(duplicates, (group) => duplicateKey(allowed) === duplicateKey(group))
      )
    })
  ...
  const stale = Arr.filter(
    ...
    (unit) =>
      !Arr.some(
        currentRecord.units,
        (reviewed) => key(reviewed) === key(unit) && reviewed.semanticHash === unit.semanticHash
      )
  )
  const removed = Arr.filter(
    currentRecord.units,
    (reviewed) => !Arr.some(inventory.units, (unit) => key(reviewed) === key(unit))
  )
  const unreviewedDuplicates = Arr.filter(
    duplicates,
    (group) => !Arr.some(currentRecord.duplicateAllowlist, (allowed) => duplicateKey(allowed) === duplicateKey(group))
  )
  const diagnostics = [
...
}).pipe(Effect.catchTag("ReviewError", (error) =>
  Effect.zipRight(
    Effect.forEach(error.diagnostics, Effect.logError),
    Effect.fail(error)
  )))
