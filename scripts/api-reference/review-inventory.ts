...
export const buildInventory = Effect.gen(function*() {
  ...
  const loaded = yield* Effect.forEach(docsManifest.packages, (pkg) =>
    Effect.forEach(
      pkg.apiModules,
      (summary) =>
        Effect.gen(function*() {
          ...
          return {
            ...
            summary,
            index,
            exports: Arr.map(pages, (_) => _.export)
          }
        }),
      { concurrency: 8 }
  ...
  const entrypointUnits = yield* Effect.forEach(
    entrypoints,
    ({ pkg, summary, index, exports }) =>
      Effect.gen(function*() {
        const sourcePackage = Arr.findFirst(manifest.packages, (_) => _.name === pkg.name)
        const sourceModule = Option.flatMap(sourcePackage, (_) =>
          Arr.findFirst(_.modules, (module) =>
            Arr.some(module.routes, (route) =>
              route.canonical && route.path === index.path)))
        const sourceRoutes = Option.match(sourceModule, {
          onNone: () => [],
          onSome: (_) =>
            _.routes
        })
        ...
        const metadata = Arr.flatMap(index.exports, (entry, position) => {
          ...
          const reflected = Option.flatMap(canonicalRoute, (_) =>
            Arr.findFirst(_.imports, (candidate) =>
        ...
        const authored = [
          ...
          ...Arr.flatMap(exports, (entry) => [
            ...
            ...Arr.filterMap([entry.summary], (text) =>
              Option.fromNullable(proseDiagnostic(entry.id, text)))
          ])
        ]
        const counts = addCounts(
          Arr.reduce(Arr.map(exports, exportCounts), {
            ...zeroCounts(),
            modules: 1,
            routes: 1 + index.aliases.length,
            projections: Arr.reduce(sourceRoutes, 0, (count, route) =>
              count + route.imports.length),
            categories: index.categories.length
          }, addCounts),
          docsCounts([index.module.docs])
        )
        ...
        return {
          ...
          summaries: Arr.map(exports, (_) => ({
            owner: _.id,
            summary: _.summary,
            sources: Arr.map(_.facets, (facet) =>
              facet.sourceUrl)
          })),
    ...
    ({ pkg, summary, index }) =>
      Effect.gen(function*() {
        ...
        const sourceModule = Option.flatMap(sourcePackage, (_) =>
          Arr.findFirst(_.modules, (module) =>
            Arr.some(module.routes, (route) =>
              route.canonical && route.subpath === summary.subpath)))
        const canonicalRoute = Option.flatMap(sourceModule, (module) =>
          Arr.findFirst(module.routes, (route) =>
            route.canonical && route.subpath === summary.subpath))
        const expectedImports = Option.match(canonicalRoute, {
          onNone: () => [],
          onSome: (route) =>
            Arr.filter(route.imports, (entry) =>
  ...
  const totals = Arr.reduce(
    publicUnits,
    { ...zeroCounts(), packages: docsManifest.packages.length },
  ...
  const importPaths = HashMap.fromIterable(
    Arr.flatMap(manifest.packages, (pkg) =>
      Arr.flatMap(pkg.modules, (module) =>
        Arr.map(module.routes, (route) => [
