...
describe("API documentation examples", () => {
  it.effect("strictly compiles a canonical public import", () => Effect.sync(() => {
  ...
  it.effect("attributes compiler diagnostics to the owning export", () => Effect.sync(() => {
    expect(exampleDiagnostics(process.cwd(), [{
      owner: "digest#encodeUtf8",
      package: "@scenesystems/digest",
      language: "ts",
  ...
  it.effect("rejects relative, internal, and context-free examples", () => Effect.sync(() => {
    const diagnostics = exampleDiagnostics(process.cwd(), [{
      owner: "digest#bad",
      package: "@scenesystems/digest",
      language: "ts",
      code: "import { hidden } from \"@scenesystems/digest/internal/hidden\"\nimport { local } from \"./local.js\"\nvoid hidden\nvoid local"
    }], supported)
    expect(diagnostics).toContain("digest#bad: @example imports a private, relative, or Node module")
    expect(diagnostics).toContain("digest#bad: @example imports unsupported path @scenesystems/digest/internal/hidden")
