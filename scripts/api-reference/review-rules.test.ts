...
describe("API review rules", () => {
  it.effect("rejects null and unresolved authored links", () => Effect.sync(() => {
  ...
  it.effect("detects exact duplicates by distinct owner", () => Effect.sync(() => {
  ...
  it.effect("does not treat short vaguely similar summaries as duplicates", () => Effect.sync(() => {
    expect(duplicateGroups([
  ...
  it.effect("keeps independent duplicate components separate", () => Effect.sync(() => {
    expect(duplicateGroups([
  ...
  it.effect("does not require exceptions for projections of one declaration", () => Effect.sync(() => {
  ...
  it.effect("uses narrow prohibited-prose rules", () => Effect.sync(() => {
  ...
  it.effect("enforces the controlled public category vocabulary", () => Effect.sync(() => {
    expect(categoryDiagnostic("pkg#a", "schemas")).toBeUndefined()
    expect(categoryDiagnostic("pkg#a", "utils")).toBeDefined()
  }))

  it.effect("separates summaries from remarks and qualifies deprecations", () => Effect.sync(() => {
  ...
  it.effect("requires search entries to match canonical presentation metadata", () => Effect.sync(() => {
    const expected = {
      id: "pkg#value",
      package: "@scope/pkg",
      packageSlug: "pkg",
