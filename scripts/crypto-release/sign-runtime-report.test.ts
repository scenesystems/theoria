...
const MISMATCHED_MODULE_MODES: ReadonlyArray<readonly [SignRuntimeModuleMode, SignRuntimeModuleMode]> = [
  ["esm", "cjs"],
  ["cjs", "esm"]
]
...
describe("Sign packed runtime report qualification", () => {
  it.effect("preserves a bounded nonzero child process diagnostic", () =>
    Effect.gen(function*() {
  ...
  it.effect("classifies successful schema-invalid stdout as an invalid runtime report", () =>
    Effect.gen(function*() {
      const error = yield* runChild("bun", "esm", writeStdout("{}")).pipe(Effect.flip)
  ...
  it.effect.each(MISMATCHED_MODULE_MODES)(
    "rejects a reported %s mode when %s was requested",
    ([requestedMode, reportedMode]) =>
      Effect.gen(function*() {
        const error = yield* runChild("bun", requestedMode, writeStdout(validReport(reportedMode))).pipe(Effect.flip)

        expect(error).toMatchObject({
          _tag: "CryptoReleaseCheckError",
          stage: `sign-packed-bun-${requestedMode}`,
          detail: `runtime report module mode ${reportedMode} did not match requested ${requestedMode}`
        })
      })
  )
